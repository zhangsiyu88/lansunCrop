package com.lansun.qmyo.utils.gooview;
...
public class GooView extends View {
	...
	public GooView(Context context) {
		...
		stickCircleRadius = Utils.dip2Dimension(5.0f, context); //拖拽圆的半径
		dragCircleRadius = Utils.dip2Dimension(5.0f, context); //固定圆的半径
		stickCircleMinRadius = Utils.dip2Dimension(3.0f, context); //最远处的最小的圆的半径，当半径小于此值时，连结圆会断裂掉
		farest = Utils.dip2Dimension(80.0f, context);    //所画圆弧的最远半径
		resetDistance = Utils.dip2Dimension(40.0f, getContext());  //重置小点的距离
		
		mPaintRed = new Paint(Paint.ANTI_ALIAS_FLAG);//抗锯齿
		
		/*mPaintRed.setColor(Color.RED);*/
		mPaintRed.setColor(Color.rgb(157, 193, 79));//绿色画笔
...
//		path.addCircle((float)stickPoints[1].x, (float)stickPoints[1].y, 5, Direction.CW);
		...
		shapeDrawable.getPaint().setColor(Color.rgb(157, 193, 79));
	...
	protected void onDraw(Canvas canvas) {
		...
		if(!isDisappear){
			...
			canvas.drawText("", mDragCenter.x , mDragCenter.y + dragCircleRadius /2f, mTextPaint);
		}
