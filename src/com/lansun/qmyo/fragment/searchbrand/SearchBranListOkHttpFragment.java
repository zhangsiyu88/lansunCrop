...
public class SearchBranListOkHttpFragment extends BaseFragment implements OnClickListener,TextWatcher{
	...
	private void getTitleBanner() {
		...
		startProgress();
		// 服务板块
		refreshCurrentList(GlobalValue.URL_SEARCH_HOLDER_SERVICE+"000000",null, 0, expandtab_view);
		// 附近 固定
		Log.e("select_cityCode", App.app.getData("select_cityCode"));
		refreshAndroidOrder(GlobalValue.URL_SEARCH_HOLDER_DISTRICT+App.app.getData("select_cityCode"),1,null, expandtab_view);
		// 智能排序
		refreshAndroidOrder(GlobalValue.URL_SEARCH_HOLDER_INTELLIGENT, 2, null, expandtab_view);
		// 筛选
		refreshAndroidOrder(GlobalValue.URL_SEARCH_HOLDER_SCREENING, 3, null, expandtab_view);
	}
	/**
	 * 设置Fragment
	 * @param fragment
	...
	private void result(ResponseEntity r) {
		if (r.getStatus() == FastHttp.result_ok) {
			...
			switch (r.getKey()) {
			case 0:// 全部
				Log.e("AllService",r.getContentAsString());
				...
				break;
			case 1:// 附近
				Log.e("nearService",r.getContentAsString());
				...
				break;
			case 2:// 智能排序
				Log.e("intelligent", r.getContentAsString());
			...
			case 3:// 筛选
				Log.e("sxintelligent", r.getContentAsString());
				...
				if (holder_button.size() == 4) {
					...
					.setOnSelectListener(new ViewLeft.OnSelectListener() {
						...
						public void getValue(String showText, int parendId,
								int position) {
							if (parendId == 0) {
								if (nearService.getData().get(parendId)
										.getItems() == null) {
									onRefresh(viewLeft, showText);
									SearchBranListOkHttpFragment.this.position = nearService
				...
				if (TextUtils.isEmpty(list.getNext_page_url())) {
					...
					try {
						refreshParams.put("poistion", URLEncoder.encode(
								SearchBranListOkHttpFragment.this.position, "utf-8"));
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
					try {
