...
public class ActivityFragment extends BaseFragment {
	...
	private void loadActivityList() {
		...
		refreshKey = 4;
		refreshCurrentList(refreshUrl+"site="+getSelectCity()[0]+"&service="+HODLER_TYPE+"&position="
									+position_bussness+"&intelligent="+intelligentStr+"&type="+type+"&location="+GlobalValue.gps.getWgLat()+","
									+GlobalValue.gps.getWgLon()+"&query="+"",null, refreshKey,lv_activity_list);
		
		//TODO
		Log.d("八大板块测试", "八大板块页面准备前去访问数据：  "+System.currentTimeMillis());
	}
	...
	private void result(ResponseEntity r) {
		...
		if (r.getStatus() == FastHttp.result_ok) {
			...
			case 3:// 筛选
				Log.d("八大板块测试", "已获取筛选的数据 "+System.currentTimeMillis());
				Log.i("筛选的数据", "拿到筛选的数据!");
			...
			case 4:// 活动列表
				...
				Log.d("八大板块测试", "八大板块页面去访问数据现已拿到数据内容的时间： "+System.currentTimeMillis());
				try{
